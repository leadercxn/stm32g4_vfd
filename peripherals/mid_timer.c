                ...
                if(p_timer_list_node->p_timer_node->remain_ms > 0)
                {
                    ...
                    if(p_timer_list_node->p_timer_node->remain_ms == 0)
                    {
                        ...
                        else
                        {
                            ...
                            if(err_code)
                            {
                                trace_error("app_sched_event_put error %d, start idx %d, end idx %d\n\r", err_code, m_timer_scheduler.m_queue_start_index, m_timer_scheduler.m_queue_end_index);
                            }
                        }
                    }
                }
...
{
    systick_timer_handler_register(per_ms_timer_handler);
